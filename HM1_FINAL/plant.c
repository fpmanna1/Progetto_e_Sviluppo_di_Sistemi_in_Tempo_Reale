...
static int keep_on_running = 1; // finchè questa variabile è 1 i nostri task eseguono

struct shared_int {
	int value;
	pthread_mutex_t lock;
}; 
static struct shared_int shared_sensor; // sensor ed actuator nello schema
static struct shared_int shared_actuator;

void init(pthread_attr_t* attr, struct sched_param* myparam, periodic_thread* plant, periodic_thread* sensor);
...
void * sensor_driver_loop(void* par) {
	...
	while (keep_on_running) {
		...
		if (mq_send (sensor_qd, message, strlen (message) + 1, 0) == -1) {
		    ...
		    continue; // in questo ramo potrei entrarci se inviassi 1000 messaggi senza prelevarne nessuno
		}
	}
...
void * actuator_driver_loop(void* par) {
	// è un task ciclico ma non è implementato come task periodico
	// il comportamento ciclico è implementato attraverso una receive bloccante
	// è un task periodico se arrivano periodicamente messaggi
	...
	while (keep_on_running) {
		...
		if (mq_receive(actuator_qd, message,MAX_MSG_SIZE,NULL) == -1){ // receive bloccante
...
int main(void)
{
	...
	init(&myattr, &myparam, &plant_th, &sensor_th);
	...
	pthread_setaffinity_np(plant_thread, sizeof(cpu_set_t), &cpuset); 
    
	pthread_create(&sensor_driver_thread,&myattr,sensor_driver_loop,(void*)&sensor_th);
	pthread_setaffinity_np(sensor_driver_thread, sizeof(cpu_set_t), &cpuset);
	
    
	pthread_create(&actuator_driver_thread,&myattr,actuator_driver_loop,NULL); 
	pthread_setaffinity_np(actuator_driver_thread, sizeof(cpu_set_t), &cpuset);
...
void init(pthread_attr_t* attr, struct sched_param* myparam, periodic_thread* plant, periodic_thread* sensor)
{
	...
	plant->period = TICK_TIME;
	plant->priority = 50;

	myparam->sched_priority = plant->priority;
	pthread_attr_setschedparam(attr, myparam);


	sensor->period = TICK_TIME;
	sensor->priority = 50;

	myparam->sched_priority = sensor->priority;
	pthread_attr_setschedparam(attr, myparam); 
